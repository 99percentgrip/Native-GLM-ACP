...
| `/mobile` | Start or stop the optional scan-to-approve companion. Start chat with `--mobile-bind 0.0.0.0:8765 --mobile-allow-public`, then use `/mobile` and scan the immediate pairing QR once; the paired phone receives later Allow/Deny requests without another scan. Loopback remains the safe default; use `--mobile-url` for an explicitly supplied Tailscale/reverse-proxy URL. |
...
| `/max-iterations [N]` | Show (no arg) or set (e.g. `/max-iterations 100`) the per-turn tool-call iteration cap. Default 50, max 1000. Clamped silently. **Persists across sessions** — `/max-iterations N` writes a user default to `~/.config/glm-acp/max-iterations.json` so a fresh `glm-acp chat` launch starts with the saved cap. **Works in all three frontends**: ACP editors (Zed and other ACP-compatible editors), `glm-acp chat` (TUI), and `glm-acp chat --plain`/`--prompt`/`--stdin`/`--json`. Resolution precedence: `GLM_ACP_MAX_TOOL_ITERATIONS` env var wins for ad-hoc runs; otherwise the persisted file; otherwise 50. |
...
To pin a release, set `GLM_ACP_VERSION=v2.7.32` before running the Unix
installer, or pass `-Version v2.7.32` to the downloaded PowerShell script.
The current release and manual-download fallback is
[v2.7.32](https://github.com/99percentgrip/Native-GLM-ACP/releases/tag/v2.7.32).
...
# expect: glm_acp-2.7.32.dist-info  (and editable-install metadata)
```
