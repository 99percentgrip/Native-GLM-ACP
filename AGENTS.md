...
- Package and ACP implementation version is `2.4.0` from `glm_acp.__version__`.
- GitHub release `v2.4.0` publishes the five supported frozen binaries, checksums, provenance attestations, Python distributions, Registry metadata, the icon, checksum-verifying Unix and Windows installers, and safe one-command uninstall support.
...
# expect: editable glm_acp metadata and glm_acp-2.4.0.dist-info
```
