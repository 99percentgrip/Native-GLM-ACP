...
- Package and ACP implementation version is `1.9.5` from `glm_acp.__version__`.
- GitHub release `v1.9.5` publishes the five supported frozen binaries, checksums, provenance attestations, Python distributions, Registry metadata, the icon, checksum-verifying Unix and Windows installers, and safe one-command uninstall support.
...
# expect: editable glm_acp metadata and glm_acp-1.9.5.dist-info
```
