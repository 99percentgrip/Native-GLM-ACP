...
- Standalone terminal selection is editor-parity oriented: a Codex-style right-click context menu opens via Ctrl+Right Click (Shift+Right Click as a terminal-friendly fallback when the terminal intercepts Ctrl+Click, F6 as a pure-keyboard fallback because Ctrl+M is byte-equivalent to Enter in terminals) and offers Cut / Copy / Paste / Select All in the composer and Copy selection / Select all output / Copy last response / Copy all responses in the transcript area; Ctrl+Shift+C copies the current Textual selection and Ctrl+Y copies the last agent response, all through credential-safe OS clipboard helpers with a one-million-character bound. Agent-message widgets use a `SelectableStatic` subclass that exposes the raw markdown source for selection — Textual's default `Static.get_selection` returns `None` for Rich renderables (RichMarkdown), so without this subclass agent responses are invisible to selection.
...
- Package and ACP implementation version is `2.0.5` from `glm_acp.__version__`.
- GitHub release `v2.0.5` publishes the five supported frozen binaries, checksums, provenance attestations, Python distributions, Registry metadata, the icon, checksum-verifying Unix and Windows installers, and safe one-command uninstall support.
...
# expect: editable glm_acp metadata and glm_acp-2.0.5.dist-info
```
